core.py – גרסה משולבת ומדויקת של לוגיקת הבוט

import requests import time import datetime import pytz import matplotlib.pyplot as plt import csv from settings import ( TELEGRAM_TOKEN, CHAT_IDS, TWELVE_API, FINNHUB_API, TOTAL_BUDGET, ALLOCATED, stocks, USE_ANALYST_RATING, TEST_MODE, USE_RSI, USE_NEWS, USE_MACD, USE_VOLUME_SPIKE, ALPHA_VANTAGE_API, NEWS_API, TRADER_PROFILE )
...
def get_macd(symbol): try: url = f"https://finnhub.io/api/v1/indicator?symbol={symbol}&resolution=1&indicator=macd&token={FINNHUB_API}" r = requests.get(url).json() return round(r["macd"]["macd"][-1] - r["macd"]["signal"][-1], 4) except: return None
...
def create_graph(symbol, prices): if len(prices) < 2: return None plt.figure() plt.plot(prices, marker='o') plt.title(f"{symbol} – Last Prices") plt.axhline(y=stocks[symbol]["low"], color='green', linestyle='--', label='Support') plt.axhline(y=stocks[symbol]["high"], color='red', linestyle='--', label='Resistance') plt.legend() path = f"{symbol}.png" plt.savefig(path) plt.close() return path
