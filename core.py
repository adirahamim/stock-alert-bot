...
import csv
from settings import TELEGRAM_TOKEN, CHAT_IDS, TWELVE_API, FINNHUB_API, TOTAL_BUDGET, ALLOCATED, stocks, USE_ANALYST_RATING, TEST_MODE, USE_RSI, ALPHA_VANTAGE_API, USE_NEWS, NEWS_API, USE_MACD, USE_VOLUME_SPIKE, TRADER_PROFILE

last_summary_time = None
already_reported = {}
MAX_PRICE_HISTORY = 50
CSV_LOG_PATH = "signals_log.csv"
...
def send_telegram_image(image_path, caption=""):
    ...
    for chat_id in CHAT_IDS:
        with open(image_path, "rb") as image_file:
            requests.post(url, files={"photo": image_file}, data={"chat_id": chat_id, "caption": caption})
...
def get_macd(symbol):
    try:
        ...
        macd_val = r["macd"]["macd"][-1]
        signal_val = r["macd"]["signal"][-1]
        return round(macd_val - signal_val, 4)
    except:
        return None
...
def get_volume_spike(symbol):
    try:
        ...
        r = requests.get(url).json()
        if r.get("v") and len(r["v"]) >= 10:
            last_volume = r["v"][-1]
            avg_volume = sum(r["v"][-10:-1]) / 9
...
def get_news_summary(symbol):
    if not USE_NEWS:
        return None
    try:
        url = f"https://newsapi.org/v2/everything?q={symbol}&apiKey={NEWS_API}&language=en&sortBy=publishedAt&pageSize=1"
        res = requests.get(url).json()
        if res["articles"]:
...
def decide_action(symbol, price, rating, rsi):
    recommendation = ""
...
def run_bot():
    ...
    print("✅ הבוט פועל... בודק מניות בדיוק מקסימלי")
    send_telegram("🚀 הבוט התחיל לרוץ – מצב דיוק מקסימלי פעיל")
