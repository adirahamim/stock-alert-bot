...
def get_volume_spike(symbol): url = f"https://finnhub.io/api/v1/stock/candle?symbol={symbol}&resolution=1&count=30&token={FINNHUB_API}" r = requests.get(url).json() if r.get("v") and len(r["v"]) >= 2: last_volume = r["v"][-1] avg_volume = sum(r["v"]["v"][-10:-1]) / 9 return last_volume > avg_volume * 1.5 return False
...
while True:
    ...
    for symbol in stocks:
        ...
        send_telegram_image(graph_path, current_summary)
